...
public class EventsController {
    ...
    public String showEvents(@RequestParam (required = false) Double price, @RequestParam (required = false)  EventType type, Model model) {
        ...
        model.addAttribute("type",eventTypeList);
        if (price == null && type == null) {
           model.addAttribute("events",this.eventService.listAllEvents());
        } else {
            model.addAttribute("events",  this.eventService.listEventsWithPriceLessThanAndType(price, type));
        }
        return "list.html";
    }
